...
ruby -ryaml -rdate - "$TARGETS_FILE" "$user_id" > "$tmp_sql" <<'RUBY'
def q(value)
...
payload.fetch("checks").each do |check|
  ...
  when "http"
    ...
    columns = "name, active, user_id, interval, retry_interval, url, type, accepted_statuscodes_json, method, description, timeout"
    values = "#{q(name)}, 1, #{user_id}, #{interval}, #{interval}, #{q(target)}, 'http', #{q(expected)}, 'GET', #{q(description)}, 48"
  when "tcp"
    ...
    columns = "name, active, user_id, interval, retry_interval, hostname, port, type, accepted_statuscodes_json, method, description, timeout"
    values = "#{q(name)}, 1, #{user_id}, #{interval}, #{interval}, #{q(host)}, #{int(port)}, 'port', #{q(expected)}, 'GET', #{q(description)}, 48"
  else
